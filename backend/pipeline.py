...
def init(): # andrà prob spostato in dockerfile
    ...
    os.makedirs('./roboflow-model', exist_ok=True)
    os.makedirs('./sdxl_lora_weights', exist_ok=True)
    ...
    os.makedirs('./data/stage_2', exist_ok=True)
    os.makedirs('./data/stage_2/masks', exist_ok=True)
    os.makedirs('./data/stage_2/cropped_masks', exist_ok=True)
...
def start(input_image: Image.Image, team1: str, team2: str):
    ...
    # Crop players from images (filter out referees, goalkeepers and balls)
    pad = 50
    i = 0
    for b in boxes:
    ...
    i = 0
    result_stage_2 = model(input_image, bboxes=copy.deepcopy(boxes))[0]     # deep copy to avoid array "contamination"

    # Process result
    i = 0
    for m in result_stage_2.masks:
        if classes[i] == 2:       # only players
            mfile = Image.fromarray((m.data.cpu().numpy().squeeze() * 255).astype(np.uint8)).convert('L')   # create mask image
            mfile.save(fp=f"data/stage_2/masks/mask_{i}.jpg")                                               # save to disk
            b = boxes[i].tolist()
            cmfile = mfile.crop((b[0] - pad, b[1] - pad, b[2] + pad, b[3] + pad))                           # create cropped mask image                                                                          # display to screen
            cmfile.save(fp=f"data/stage_2/cropped_masks/cropped_mask_{i}.jpg")                              # save to disk
        i += 1

    # ----------------------------

    ## STAGE 2b
    # questo modello divide i giocatori in due squadre

    crops = []
    # i crop glieli do io - oppure lo allena sul momento con i calciatori della foto in input?
    ...
    for player_box in boxes:
        ...
        mask_image = dilated_mask(Image.open(f"data/stage_2/cropped_masks/cropped_mask_{i}.jpg").convert('L').resize(output_inpaint_size), 6) # load, dilate and resize mask
    ...
    for player_box in boxes:
        ...
        mask_image = dilated_mask(Image.open(f"data/stage_2/cropped_masks/cropped_mask_{i}.jpg").resize((player_box[2] - player_box[0] + pad * 2, player_box[3] - player_box[1] + pad * 2)), 2).convert('L')
        # Incolla l'immagine
